...
# VOICE CASTING. The palette above is not interchangeable — these are six timbres with real and
# very different pitches, measured: onyx ~98 Hz, echo ~101, alloy ~148, fable ~154, shimmer ~160,
# nova ~219. Dealing them by `hash(id) % 6` therefore did not produce "a distinct voice per
# character", it produced a random one: in Ashford, Marta the blacksmith and Greta the market woman
# — the town's only two women — drew "fable" and "onyx", the latter being the deepest man in the
# set, and three of the four NPCs collided on onyx so most of the town spoke as the same deep man.
#
# So cast from the character, not from a hash of its name. An NPC declares `gender` and `age` in
# its world.json record; the hash then only chooses WITHIN the matching band, which is what keeps a
# character sounding the same every session while still letting two villagers differ.
#
# NO STRING SNIFFING. It is tempting to guess gender from the persona text, and it would work until
# the next phrasing and then fail silently and unfixably. An NPC with no declared band keeps the
# old whole-palette deal — the same voice it has today, so nothing existing shifts under the player
# — and the playbook asks new builds to declare it.
const VOICE_BANDS := {
	"female": ["nova", "shimmer"],
	"male": ["onyx", "echo"],
	"neutral": ["alloy", "fable"],
}
# A child is a PERFORMANCE, not a timbre — there is no child voice in the palette, and the highest
# (nova, ~219 Hz) is mid adult-female against a child's 250-400 Hz. The worker drives gpt-audio-1.5
# through a voice-actor system prompt, so `age` is forwarded and directed there; here it only
# narrows the timbre to the brightest two.
const CHILD_VOICES := ["nova", "shimmer"]
...
var dlg_queue: Array = []
var active := false
var dlg_hold := 0.0            # seconds the LAST line stays up before it fades itself out (see _advance)
...
func clear() -> void:
	...
	_speak_queue.clear()
	# RELEASE THE DUCK BEFORE DROPPING _speaking, or the mix never comes back up.
	# duck_for_speech is reference-counted and _speak_next's only release sits behind
	# `if _speaking:` in the empty-queue branch. Clearing the flag here made that branch
	# unreachable, and voice_player.stop() emits no `finished` in Godot 4, so
	# _on_voice_finished never ran either — one hold leaked per area transition taken
	# mid-sentence. Music and SFX stayed 9 dB down for the rest of the session; cross two
	# seams and the next duck drove them to -18.
	if _speaking:
...
func _physics_process(delta: float) -> void:
	# The last line of a message times itself out. Ticked BEFORE the early returns so a notice
	# cannot be left frozen on screen by a scene transition or a missing player.
	if dlg_hold > 0.0:
		dlg_hold = maxf(0.0, dlg_hold - delta)
		if dlg_hold == 0.0:
			_dismiss()
...
func add_npc(pos: Vector3, npc_id: String, npc_name: String, persona: String, lines: Array, model: Node = null, parent: Node = null, cell_key := "", sound := "", gender := "", age := "", activity := "", holds: Node = null, seated := false) -> void:
	...
	if sound != "":
		AudioManager.attach_loop(npc_body, AudioManager.stream_for("res://audio/%s.ogg" % sound), -10.0, 12.0, 4.0)
...
func add_seam(pos: Vector3, to_area: String, spawn: String, lock: String, label: String, id := "") -> void:
	var col := Color(0.35, 0.5, 0.75) if lock == "" else Color(0.55, 0.32, 0.18)
	var node := _box(pos + Vector3(0, 1.6, 0), Vector3(3.0, 3.2, 0.5), col)
	# `id` is what the rule layer addresses (`unlock_seam`). Falls back to the label so a world that
	# never names its seams is still reachable by the words the player sees on the prompt.
	items.append({kind = "seam", pos = pos, node = node, label = label,
		id = (id if id != "" else label), to = to_area, spawn = spawn, lock = lock})
...
# A PERSISTENT world-anchored custom interactable (map beacons etc.) — never cell-parented,
...
func try_use() -> void:
	...
	if it == null:
		return
	# `interact` — ONE event for "the player used the thing they were standing at", carrying what
	# it was. Raised before the handler so a rule can react to a chest opening in the same frame,
	# and raised for every kind so an author never has to ask which ones are wired.
	fire("interact", {"id": String(it.get("id", it.get("label", ""))), "kind": String(it.kind)})
	match it.kind:
...
# ---------------- rule-layer API (`open_door` / `close_door` / `unlock_seam`) ----------------
#
# Addressed by `id` (authored) falling back to `label` (what the prompt says), and "" means every
# door / every seam. Each returns how many entries it touched so rules.gd can report a name that
# matched nothing rather than logging a success for a typo — the same contract as `hud_hide`.


func _addresses(it: Dictionary, key: String) -> bool:
	return key == "" or String(it.get("id", "")) == key or String(it.get("label", "")) == key


## Open or close doors by name. Covers BOTH door kinds: the hinged doors a chunk world places and
## the structure doors a building generates. A rule that says "open the gate" should not have to
## know which subsystem built it.
func set_door_open(key: String, want_open: bool) -> int:
	var n := 0
	for it in items:
		if not (it is Dictionary):
			continue
		var k := String((it as Dictionary).get("kind", ""))
		if k == "door" and _addresses(it, key):
			_swing_door(it, want_open)
			n += 1
		elif k == "sdoor" and _addresses(it, key):
			if bool((it as Dictionary).get("open", false)) != want_open:
				_toggle_structure_door(it)
			n += 1
	return n


## Clear the lock on a seam or door, so it opens without the key. The world-changing half of a
## quest reward: "the guard waves you through" is this, not a hidden item grant.
func unlock(key: String) -> int:
	var n := 0
	for it in items:
		if not (it is Dictionary):
			continue
		var k := String((it as Dictionary).get("kind", ""))
		if (k == "seam" or k == "door") and _addresses(it, key) and String((it as Dictionary).get("lock", "")) != "":
			(it as Dictionary)["lock"] = ""
			n += 1
	return n


## The rule layer's funnel. Null-safe: a world with no rules pays one branch.
func fire(event: String, payload: Dictionary) -> void:
	if main_ref == null:
		return
	var d = main_ref.get("director")
	if d != null and d.has_method("fire"):
		d.fire(event, payload)
...
func _open_door(it: Dictionary) -> void:
	if it.get("open", false):
		# TOGGLE — USE on an open door shuts it. NO NOTICE: `AudioManager.play_sfx("door")` in
		# _swing_door already says it happened, the leaf visibly swings, and the USE prompt flips
		# its own label. Narrating it added nothing and cost the player an input (see _show).
		_swing_door(it, false)
		return
...
## Open or close a hinged door. Extracted from _open_door so the rule actions `open_door` /
## `close_door` drive the SAME code the player's USE does — a parallel implementation is how the
## two drift until a rule-opened door has no collider change and the player walks into thin air.
##
## Closing re-enables the collider only when the leaf is shut, never mid-swing, so a player standing
## in the doorway is not caught inside a sweeping shape (the rule structure doors already follow).
func _swing_door(it: Dictionary, want_open: bool) -> void:
	if bool(it.get("open", false)) == want_open:
		return
	it.open = want_open
	AudioManager.play_sfx("door")
	var pv = it.get("pivot")
	if want_open and is_instance_valid(it.get("shape")):
		(it.shape as CollisionShape3D).disabled = true   # walk through now
	if pv != null and is_instance_valid(pv):
		var node := pv as Node3D
		var old_tw = it.get("tween")
		if old_tw != null and (old_tw as Tween).is_valid():
			(old_tw as Tween).kill()
		var closed := float(it.get("closed_yaw", node.rotation.y))
		var tw := node.create_tween()
		it["tween"] = tw
		tw.tween_property(node, "rotation:y", closed + (deg_to_rad(95.0) if want_open else 0.0), 0.45)
		if not want_open:
			tw.tween_callback(func() -> void:
				if bool(it.get("open", false)):
					return              # reopened while closing — leave it passable
				if is_instance_valid(it.get("shape")):
					(it.shape as CollisionShape3D).disabled = false)
	elif not want_open and is_instance_valid(it.get("shape")):
		(it.shape as CollisionShape3D).disabled = false
...
func _talk(it: Dictionary) -> void:
	talks += 1
	fire("talk", {"id": String(it.get("npc_id", "")), "name": String(it.get("npc_name", ""))})
	...
	for line in it.lines:
		_enqueue_speech(String(line), v, String(it.get("npc_name", "")),
			String(it.get("gender", "")), String(it.get("age", "")), String(it.get("persona", "")))
	if not it.asked:
		it.asked = true
...
## `active` MEANS "USE WILL ADVANCE TO A FURTHER LINE" — not "a message is on screen".
##
## It used to mean the second thing, and that made every system message a modal that ate the
## player's next USE. `_show` set `active = true`; `_advance(true)` does not pop, so a ONE-LINE
## message left the queue non-empty and `active` true with no timer of any kind; and `try_use`
## (:338) short-circuits `if active: _advance(); return` BEFORE it ever reaches `match it.kind`.
##
## Doors are where that was unmissable, because a door is the one interactable you use twice in a
## row: USE opened it and printed "The door swings open.", USE again only dismissed that text, and
## USE a third time finally shut it. The player reads that as "the doors don't close". The notices
## are gone now (see `_open_door`), but deleting two strings would only have retired the instance —
## ANY notice fired by an interaction stole the next USE from that same interaction.
##
## So the flag gets a lifetime. When the last line is up there is nothing left to advance to, so
## `active` is false, the panel stops swallowing taps, and USE goes straight back to the world while
## the text fades on its own. A genuine multi-line dialogue (chest contents, a locked door and its
## reason) still blocks, because there it is correct: there IS more to read.
...
func _advance(first := false) -> void:
	...
	active = dlg_queue.size() > 1
	# A panel that cannot be advanced must not eat the tap either — MOUSE_FILTER_STOP spans the
	# whole top band, so the last line would otherwise keep swallowing touches where no advance is
	# possible.
	dlg_box.mouse_filter = Control.MOUSE_FILTER_STOP if active else Control.MOUSE_FILTER_IGNORE
	# Read time, not a fixed constant: a long line needs longer than "You opened the chest."
	dlg_hold = 0.0 if active else maxf(1.8, 0.045 * float(dlg_label.text.length()))


## The single place the dialogue channel goes quiet, so no path can leave `active` set behind it.
func _dismiss() -> void:
	dlg_queue.clear()
	active = false
	dlg_hold = 0.0
	dlg_box.visible = false
	dlg_box.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func _enqueue_speech(text: String, voice: String, name: String, gender := "", age := "", persona := "") -> void:
	var clean := _strip_speaker(text)
	if clean.strip_edges() == "":
		return
	_speak_queue.append({"text": clean, "voice": voice, "name": name,
		"gender": gender, "age": age, "persona": persona})
	if not _speaking:
		_speak_next()
...
func _speak_next() -> void:
	...
	req.request_completed.connect(func(_r: int, c: int, _h: PackedStringArray, b: PackedByteArray) -> void:
		...
		if c == 200 and b.size() > 0:
			...
			_on_voice_finished())
	# WHO is speaking, not just what. The worker drives gpt-audio-1.5 through a voice-actor system
	# prompt, so name/gender/age/persona shape the PERFORMANCE — which is the only way a child
	# reads as a child, there being no child timbre in the palette.
	var payload := JSON.stringify({
		"text": item.get("text", ""), "voice": item.get("voice", ""),
		"name": item.get("name", ""), "gender": item.get("gender", ""),
		"age": item.get("age", ""), "persona": item.get("persona", "")})
...
# Deterministic distinct voice per character (stable across a session) — cast from the character's
# declared gender/age where it has them, and only then dealt by id hash WITHIN that band. See
# VOICE_BANDS for why the old whole-palette hash was wrong.
func _npc_voice(it: Dictionary) -> String:
	var key := String(it.get("npc_id", it.get("npc_name", "")))
	var pool: Array = VOICES
	var age := String(it.get("age", "")).to_lower()
	var gender := String(it.get("gender", "")).to_lower()
	if age == "child":
		pool = CHILD_VOICES
	elif VOICE_BANDS.has(gender):
		pool = VOICE_BANDS[gender]
	if key == "":
		return pool[0]
	var h := 0
	for i in key.length():
