...
# Max INDIVIDUAL props per cell (live-node budget is 9*N). Raised 12 -> 20 with interiors: a cell
# that holds a building now has to carry the street furniture AND the room it contains, and a
# measured town sat at a mean of 8.7 with exactly one cell of 49 at the old ceiling — so the
# headroom is real and the ceiling was about to start silently eating the furniture instead.
const PROP_CAP := 20
...
func build_proxy(rec: Dictionary, gx: int, gz: int) -> Node3D:
	...
	if struct_list is Array:
		for st in struct_list:
			...
			var p := centre + _xz3(_clamp_into_cell(xz, half, "structure(proxy)"))
			# GROUND IT THE WAY THE RESIDENT RING WILL. _place_structure puts an enterable building
			# on its DOOR-side terrain; this used the centre, so the same building sat at two
			# different heights and JUMPED vertically the moment its cell promoted from proxy to
			# resident. Read the door from `st` — the original — because `shell` has had `interior`
			# erased by the line above, which is exactly what hid this.
			p.y = _ground_y(p.x, p.z)
			if _has_interior(st as Dictionary):
				var dxz := _door_world_xz(st as Dictionary, p)
	...
	if _far_d <= BUILDING_DETAIL_RADIUS:
		...
		if typeof(lm_far) == TYPE_DICTIONARY:
			...
			if lu != "" and builder.cache.has(lu):
				_place_one(root, lm_far, centre, half, "", false)
		var built_far := 0
		for pf in rec.get("props", []):
			...
			if typeof(pf) == TYPE_DICTIONARY and (pf as Dictionary).has("footprint"):
				...
				if bu != "" and builder.cache.has(bu):
					if _place_one(root, pf, centre, half, "", false):
						built_far += 1
...
# One invisible (no mesh) tall collider wall at a cell edge, spanning the terrain along the WHOLE
# edge rather than one point of it.
#
# It used to sample _ground_y at the edge MIDPOINT and place a box from gy-1.0 upward — a fixed
# one-metre tolerance for however much the ground rolls across 32 m. On a sloped border edge the
# ends are further than that from the middle, and the wall then either floats, leaving a gap the
# player walks out of the world through, or buries itself. Measured on the playbook's own example
# terrain (amplitude 10): 2.10 m of clear air under the wall, against a 1.6 m player capsule.
#
# Sampling the edge and spanning min..max keeps it ONE collider — the cheap property worth
# keeping — while making the tolerance the ground's actual range instead of a guess.
func _edge_wall(root: Node, edge_centre: Vector3, sz: Vector3) -> void:
	var along := Vector3(0.0, 0.0, 1.0) if sz.z > sz.x else Vector3(1.0, 0.0, 0.0)
	var span: float = (sz.z if sz.z > sz.x else sz.x)
	var lo := INF
	var hi := -INF
	for i in 9:
		var p := edge_centre + along * (span * (float(i) / 8.0 - 0.5))
		var h := _ground_y(p.x, p.z)
		lo = minf(lo, h)
		hi = maxf(hi, h)
	# from a metre below the lowest ground to the full wall height above the highest
	var bottom := lo - 1.0
	var top := hi + sz.y - 1.0
	var box := Vector3(sz.x, maxf(sz.y, top - bottom), sz.z)
	_collider_box(root, Vector3(edge_centre.x, bottom + box.y * 0.5, edge_centre.z), box)
...
func _road_strip(root: Node, centre: Vector3, dir: String, width: float, collide := true) -> void:
	...
	for i in range(seg_n):
		...
		# AND ROLL ACROSS THE WIDTH. h0/h1/hm are all sampled on the CENTRELINE, so the deck
		# pitched along the road and stayed dead level across it — on any cross-slope one edge
		# floats and the other buries. Measured against the rendered ground at the strip's two
		# edges for a default width-6 road: 1.01 m at the template's own amplitude 6, 1.68 m at
		# the playbook's amplitude 10, 8.49 m at 30. Sampling the two edges and rolling the
		# segment costs two more noise reads per segment and makes the deck sit on the hill.
		var hl := _ground_y(centre.x + tm, centre.z - width * 0.5) if ew \
			else _ground_y(centre.x - width * 0.5, centre.z + tm)
		var hr := _ground_y(centre.x + tm, centre.z + width * 0.5) if ew \
			else _ground_y(centre.x + width * 0.5, centre.z + tm)
		var roll := atan2(hr - hl, width)
		pos.y = (hm + (hl + hr) * 0.5) * 0.5 + 0.06
		var mi := MeshInstance3D.new()
		var bm := BoxMesh.new()
		bm.size = sz
		mi.mesh = bm
		...
		if ew:
			...
			mi.rotation.x = -roll      # and about x: rotating +x DROPS +z, so negate to lift it
		else:
			mi.rotation.x = -pitch     # rotate about x: +z end lifts with rising ground
			mi.rotation.z = roll       # and about z: rotating +z LIFTS +x, so keep the sign
		root.add_child(mi)
...
# The face a compiled building's way IN is cut into. Mason records the openings, not a door_face,
...
func _way_in_opening(rec: Dictionary) -> Dictionary:
	...
	for o in ops:
		if typeof(o) != TYPE_DICTIONARY:
			continue
		var od: Dictionary = o
		if float(od.get("sill", 0.0)) > 0.4 or bool(od.get("niche", false)):
			continue
...
func _has_interior(spec: Dictionary) -> bool:
	...
	if not ((typeof(iv) == TYPE_BOOL and iv) or typeof(iv) == TYPE_DICTIONARY):
		return false
	# ...AND GBuild HAS TO HAVE ACTUALLY BUILT ONE. This read the spec and nothing else, so on a
	# battered or tapered profile — where build_structure reaches its interior builder from the
	# `vertical` arm only, and therefore builds a solid mass — the engine still grounded the
	# building to its door-side terrain and registered a doorway into stone.
	var prof := String(spec.get("profile", "vertical")).to_lower()
	return not (prof in ["batter", "taper", "pyramid", "setback", "ziggurat"])
...
func _place_structure(root: Node, spec: Dictionary, centre: Vector3, half: float, cell_key := "") -> void:
	...
	var p := centre + _xz3(_clamp_into_cell(xz, half, "structure"))
...
func _place_parts(root: Node, part: Dictionary, pts: Array, centre: Vector3, half: float) -> void:
	...
	for i in cap:
		...
		# `scale` WORKS HERE NOW. It was read by _place_one and by the `structure` part arm, and
		# ignored by the other two — so a colonnade authored as
		# `part: {"url": "...", "scale": 3}` rendered at 1x with nothing logged, and the author
		# had no way to tell which of the five places the key is honoured.
		var psc := float(part.get("scale", 1.0))
		if psc > 0.0 and psc != 1.0:
			node.scale = Vector3(psc, psc, psc)
		root.add_child(node)
...
func _place_populate(root: Node, spec: Dictionary, centre: Vector3, half: float, cell_key := "") -> void:
	...
	for i in count:
		...
		# ACCEPT BOTH SPELLINGS. `_resolve` takes a String, so a `set` written the way every
		# `props` entry is written — dicts carrying a `url` — threw and dropped the ENTIRE
		# populate entry, crowd and all. The two shapes are indistinguishable to an author
		# reading the rest of world.json.
		var pick = bag.pop_back()
		var u := _resolve(String(pick) if typeof(pick) != TYPE_DICTIONARY else _asset_url(pick))
		if u == "" or not builder.cache.has(u) or builder.cache[u] == null:
			continue
		...
		var mdim: float = maxf(ab.size.x, maxf(ab.size.y, ab.size.z))
		# `scale` MEANS SOMETHING ELSE HERE, and that is the defect. Everywhere else in the engine
		# it is a multiplier; in this one branch it is the TARGET SIZE IN METRES that an oversized
		# model is normalised down to — and it is ignored entirely below max_size. An author who
		# writes `scale: 3` expecting three times bigger gets no change at all for a normal model
		# and THREE METRES TALL for a large one.
		#
		# `target_size` is the honest name. `scale` is still read when target_size is absent, so
		# no existing world changes behaviour — but using it that way now says so, because the
		# ambiguity is the thing worth removing.
		if mdim > float(spec.get("max_size", 4.0)):
			var tgt := 2.0
			if spec.has("target_size"):
				tgt = float(spec["target_size"])
			elif spec.has("scale"):
				tgt = float(spec["scale"])
				push_warning("GOGI_POPULATE `scale: %s` is being read as a TARGET SIZE IN METRES, "
					% str(spec["scale"]) + "not a multiplier — that is what this key has always "
					+ "meant on a populate entry. Write `target_size` to say it plainly.")
			n.scale *= tgt / mdim
...
## THE MISSING AXIS. A world could not express "inside a building, on storey k" — every placement
## path here is 2D-on-terrain: `_xz` reads a 3-element pos and throws the middle value AWAY, and
## _place_one then force-grounds whatever is left onto the heightmap. So a table could be put in a
## room only by luck of the terrain, and the ~726 furniture and utensil items already staged on R2
## had nowhere to go. Buildings were empty because the vocabulary had no word for "upstairs".
##
## `pos: [x, y, z]` now means y metres ABOVE THE GROUND at that point, and the two-element form is
## unchanged, so every existing world keeps its exact behaviour. Above the ground rather than an
## absolute height because that is what composes with a Mason building: its floors sit at
## `interior.floor_z + k * floor_height` over the terrain it was grounded to, so a chair on the
## first floor is simply that number — the same one already in the building's own record.
func _elev(p) -> float:
	if typeof(p) == TYPE_ARRAY and (p as Array).size() > 2:
		return float(p[1])
	return 0.0
...
# `physics` is FALSE on the far-proxy path. The proxy contract is "zero physics out there", and
# _strip_physics at the end of build_proxy was enforcing it by walking the tree and freeing every
# StaticBody3D it found. That works for shapes built inline — and NOT for the threaded ones: with
# _threaded_shapes on, _add_mesh_collision queues the trimesh on a WorkerThreadPool and attaches
# it when the worker finishes, which can be after the strip has already run. Every Mason building
# takes that path, because `collider: "mesh"` is mandatory on a compiled record. So the far ring
# could keep colliders for buildings the player cannot even see, and the work was wasted either
# way on up to 324 instances. Not building them is both cheaper and not a race.
var _missing_logged := {}     # url -> true, so one unresolved asset logs once, not once per instance
var _clamp_logged := {}       # same, for out-of-cell positions


# A PLINTH, so a building on a slope does not hang in the air.
#
# A building is grounded at ONE point — its doorway, so the threshold stays walkable — and then
# has a footprint. On rolling ground the downhill corners are simply above the terrain. Measured
# over 2,500 sites for a 10 x 8 building: a corner hangs at 88% of them on the template's default
# amplitude 6 and 92% on amplitude 10, worst 3.17 m and 5.29 m.
#
# THE OBVIOUS ALTERNATIVE IS NOT BETTER, which is why this is a plinth and not a re-ground.
# Raising the base to the footprint's highest terrain stops the hang and makes the DOORWAY the
# problem instead: a step over the 0.36 m the engine gives you for free at 77% and 86% of the same
# sites, worst 3.15 m and 5.24 m. Equally bad, just less visible until you try to walk in. So the
# threshold keeps its height and the gap underneath gets filled — which is what a real building
# on a hillside does.
#
# Slightly OVERSIZED on purpose (4 cm each side): a foundation that reads as a course of masonry
# rather than a seam you can see daylight through. It is visual only — the building's own collider
# already stops the player, and a second body here would fight it.
...
func _add_plinth(node: Node3D, foot: Vector2, yaw: float, base_y: float, mat: Material,
	...
	for a in 5:
		for b in 5:
			var l := Vector2(foot.x * (float(a) / 4.0 - 0.5), foot.y * (float(b) / 4.0 - 0.5)).rotated(yaw)
			lo = minf(lo, _ground_y(node.position.x + l.x, node.position.z + l.y))
	var drop := base_y - lo
	if drop <= PLINTH_MIN:
		return
	if drop > PLINTH_MAX_DROP:
...
# Clamp a cell-local position into the cell, and SAY when it moved.
#
# The clamp is the right safety net — an object outside its cell is worse than one at its edge —
# but it was silent, so every out-of-range placement in a cell collapsed onto the SAME edge point
# and the objects interpenetrated. Three props authored at [30,30], [60,45] and [9,9] in a 16 m
# cell all land on one spot, and a row wider than its cell piles the overflow into two heaps at
# the ends instead of making a colonnade. The author sees a mess with no reason given for it.
func _clamp_into_cell(xz: Vector2, half: float, what: String) -> Vector2:
	var c := Vector2(clampf(xz.x, -half + 0.5, half - 0.5), clampf(xz.y, -half + 0.5, half - 0.5))
	if absf(c.x - xz.x) > 0.01 or absf(c.y - xz.y) > 0.01:
		var ck := "%s|%.1f,%.1f" % [what, xz.x, xz.y]
		if not _clamp_logged.has(ck):
			_clamp_logged[ck] = true
			print("GOGI_PLACE_CLAMPED %s pos [%.1f, %.1f] is outside its %.0f m cell -> [%.1f, %.1f]"
				% [what, xz.x, xz.y, half * 2.0, c.x, c.y],
				" — everything out of range lands on the same point and interpenetrates")
	return c


func _place_one(root: Node, ref, centre: Vector3, half: float, cell_key := "", physics := true) -> bool:
	if typeof(ref) != TYPE_DICTIONARY:
		return false
	...
	if url == "" or not builder.cache.has(url) or builder.cache[url] == null:
		# SAY SO. A placement whose asset does not resolve produced nothing at all — no object, no
		# stand-in, no line — so an EMPTY cell and a BROKEN one look identical, in the editor and
		# on a phone. enemy.gd already prints GOGI_PLACEHOLDER for the same situation; scenery
		# just vanished. Once per url per cell, because a scatter of 40 would otherwise flood.
		var k := url if url != "" else JSON.stringify(ref)
		if not _missing_logged.has(k):
			_missing_logged[k] = true
			print("GOGI_PLACEHOLDER scenery ", k, " — no such asset in the cache; nothing placed")
		return false
	...
	n.position = centre + _xz3(_clamp_into_cell(xz, half, "prop"))
	if ref.has("rot"):
	...
	if ref.has("openings"):
		# MASON BUILDS THE LEAF NOW. _mason_doors discovers the compiled door_<mat> nodes in the
		# GLB and hangs each on a pivot; the old runtime leaf runs only for a building compiled
		# BEFORE that, which the record announces by carrying no "leaves" key at all. A record
		# that says `leaves: 0` asked for no doors and must not be overridden — that distinction
		# is the whole reason the key is a count and not a flag.
		var hung := _mason_doors(n, ref)
		_mason_fittings(n, ref, hung == 0 and not ref.has("leaves"))
		# A LEAF IS NOT A DOOR UNTIL SOMETHING SAYS SO. _mason_fittings builds the leaf into the
		# "gogi_door" group with a door_label, exactly like a parametric interior — but the only
		# code that turns that group into a USE target is _register_structure_doors, and its other
		# call site is _place_structure, the runtime-parametric structures[] path. A Mason building
		# is a landmark/prop and arrives HERE instead, so every compiled building shipped with a
		# StaticBody3D sealing its own doorway: visible door, solid collider, no way in.
		# Cell-keyed so eviction drops the registration in the same breath as the nodes. The far
		# ring passes cell_key "" and is deliberately skipped — a proxy four cells out is not a
		# USE target, and an unkeyed entry would never be evicted.
		if interaction != null and cell_key != "":
			_register_structure_doors(n, cell_key)
	...
	if not physics:
		cmode = "none"   # far proxy: build no collider at all rather than build and strip it
	...
	if interaction != null and bool(ref.get("sit", false)):
		...
		# FROM THE PROP'S OWN BASE, not from the terrain under it. This read _ground_y, so a chair
		# on an upper storey measured its seat from the ground far below: `sab.end.y - sgy` became
		# the whole storey height, the clamp pinned it to 1.2, and the seat registered near the
		# terrain — USE teleported the player down through the floor. n.position.y is already
		# grounded AND elevated by the lines above, which is exactly the surface the chair stands
		# on, so the clamp goes back to measuring the furniture.
		var sgy := n.position.y
...
func _place_ladder(root: Node, ref: Dictionary, centre: Vector3, half: float, cell_key: String) -> bool:
	...
	root.add_child(node)
	var xz := _xz(ref.get("pos", [0, 0]))
	node.position = centre + _xz3(_clamp_into_cell(xz, half, "ladder"))
	var yaw := deg_to_rad(float(ref.get("rot", 0.0)))
	node.rotation.y = yaw
	# `+=` and the storey term, matching _place_one. This ASSIGNED, so `pos: [x, 3.4, z]` put the
	# ladder's foot on the TERRAIN and silently discarded the 3.4 — a ladder authored on an upper
	# floor appeared at ground level.
	node.position.y += _ground_y(node.position.x, node.position.z) + _elev(ref.get("pos", [0, 0]))
	if interaction != null:
...
func _place_scatter(root: Node, ref, centre: Vector3, half: float, collide := true) -> void:
	...
	# SEEDED FROM THE CELL, like every other placement path. Scatter was the ONLY one left drawing
	# from the global generator — `grep randf` finds bare randf/randf_range in this file only here
	# and in the duplicate-node fallback below; populate, proxy-life and traffic all take a
	# GCast.rng_for(cell_seed). The consequence is visible in play rather than subtle: a cell is
	# scattered once as a far PROXY and again as a RESIDENT, so every tree in a forest jumped to a
	# new position the moment you walked one cell closer — and jumped again every time the cell
	# was evicted and rebuilt on the way back. Deriving the seed from the centre makes both builds
	# agree, because both are handed the same centre.
	var rng := GCast.rng_for(int(centre.x) * 73856093 + int(centre.z) * 19349663 + url.hash())
	...
	if mesh != null:
		...
		for i in range(cnt):
			var sj := rng.randf_range(0.8, 1.2) * base
			var rs := Basis().rotated(Vector3.UP, rng.randf() * TAU).scaled(Vector3(sj, sj, sj))
			var b := rs * mxf.basis
			var spot := Vector2(rng.randf_range(-half + 1.0, half - 1.0), rng.randf_range(-half + 1.0, half - 1.0))
			var sy := -drop * sj + _ground_y(centre.x + spot.x, centre.z + spot.y)   # ride the terrain
			# THE SOURCE NODE'S TRANSLATION COUNTS TOO. This baked only `mxf.basis` into the
			# instance and dropped `mxf.origin` — while `drop` (from `mxf * mesh.get_aabb()`) and
			# the collider (centred on `tab.get_center()`) both include it. So for any asset whose
			# mesh node carries an offset — common in library GLBs — the visible instance sat
			# somewhere its own ground-drop and its own collider did not.
			mm.set_instance_transform(i, Transform3D(b, Vector3(spot.x, sy, spot.y) + rs * mxf.origin))
			if body != null:
				var cs := CollisionShape3D.new()
				var bx := BoxShape3D.new()
	...
	for i in range(mini(cnt, 6)):
		...
		if d == null:
			continue
		root.add_child(d)
		d.position = centre + Vector3(rng.randf_range(-half + 1.0, half - 1.0), 0.0,
...
# "door_" earns its place here rather than being a special case: a compiled leaf is named
...
## once GSurf had something worth mandating.
##
## This replaces whatever the GLB shipped with, destructively. For a long time that was the single
## worst line in the Mason path: GSurf.surface() set an albedo COLOUR and nothing else, so a
## building that arrived carrying real baked ashlar and plank maps was flattened to grey and brown
## on placement. The fix was not to weaken the override, it was to make GSurf generate real
## patterned albedo + matching relief (surfaces.gd, `_pattern_height`).
##
## Keeping it a mandate rather than a fallback is the point: a game repo that hand-paints its own
## materials is exactly how a town drifts back to colour-per-box, and one already did. The engine
## owning the surface is what makes every Mason building in every game look like the same
## architecture. Do not soften this to "only when the GLB has no texture".
...
## Hang every leaf Mason compiled into the GLB. Returns how many, so the caller knows whether this
## building predates compiled doors.
##
## THE PIVOT IS NOT OPTIONAL. interaction.gd swings a door with `rotation:y`, which is a yaw about
## the PARENT's +Y — and every node inside a Mason GLB hangs under the glTF importer's Z-up→Y-up
## root, whose basis is a -90 degree X rotation. Yawing anything under that root tips the leaf over
## sideways instead of opening it, and no baked rotation can fix it because the offending basis
## belongs to the parent. Re-hanging the leaf under a pivot with an IDENTITY world basis is what
## makes `rotation:y` a real yaw. `n` is safe as that pivot's parent: it is the Godot-side node
## _place_one already yaws for the record's `rot`, so its +Y is world up.
##
## The hinge needs no arithmetic here. build123d writes a labelled solid's Location as the glTF
## node's translation and keeps its vertices local, so the leaf node's own origin IS the hinge axis.
func _mason_doors(n: Node3D, ref: Dictionary) -> int:
	...
	while not stack.is_empty():
		...
		for c in nn.get_children():
			stack.append(c)
		if nn is MeshInstance3D and String(nn.name).begins_with(MASON_DOOR_PREFIX):
			leaves.append(nn)
	if leaves.is_empty():
		return 0
	# A door with no interior is a door onto solid masonry. It still gets its leaf and collider so
	# the facade reads right, but it is NOT registered — opening it would reveal a wall.
	var itr = ref.get("interior", null)
	var openable: bool = typeof(itr) == TYPE_DICTIONARY or (typeof(itr) == TYPE_BOOL and bool(itr))
	for mi in leaves:
		var keep := mi.global_transform
		var pivot := Node3D.new()
		pivot.name = "MasonDoorPivot"
		n.add_child(pivot)
		pivot.global_transform = Transform3D(Basis(), keep.origin)
		mi.reparent(pivot, true)
		mi.global_transform = keep
		# The collider is synthesized from the SHIPPED mesh rather than from a second set of
		# numbers, so it cannot drift from the leaf the way the old record-sized one did.
		var ab := mi.get_aabb()
		var body := StaticBody3D.new()
		body.collision_layer = 1
		var cs := CollisionShape3D.new()
		var bs := BoxShape3D.new()
		bs.size = Vector3(maxf(ab.size.x, 0.06), maxf(ab.size.y, 0.06), maxf(ab.size.z, 0.06))
		cs.shape = bs
		cs.position = ab.position + ab.size * 0.5
		body.add_child(cs)
		mi.add_child(body)
		if openable:
			pivot.add_to_group("gogi_door")
			pivot.set_meta("door_label", "Door")
	return leaves.size()


func _mason_fittings(n: Node3D, ref: Dictionary, legacy := true) -> void:
	...
	# THE RUNTIME LEAF IS LEGACY-ONLY NOW. It was wrong in five ways at once and none of them were
	# fixable from here: it skipped every face except "s", so a way-in on n/e/w got a hole and no
	# door; it anchored at the outer wall plane so the panel straddled the facade instead of
	# hanging in the reveal; it used the interior floor for the base and ignored the opening's own
	# sill; and for an arch it hung a RECTANGLE of springing+radius, driving the top corners into
	# the spandrel. The engine never sees the spec that cut the hole — only the placement record —
	# so Mason is the only place that could have got it right.
	for o in (ref.get("openings", []) if legacy else []):
		if typeof(o) != TYPE_DICTIONARY:
			continue
...
func _place_bridge(root: Node, spec: Dictionary, centre: Vector3, half: float) -> Node3D:
	...
	for i in range(n):
		...
		# Deterministic from the plank's own index, not the global generator — the same reason
		# scatter was seeded above. A 3-degree jitter nobody notices is still a difference the
		# bridge re-rolls every time its cell rebuilds; sin of the index costs nothing and the
		# bridge looks the same every visit.
		plank.rotation.y = 0.05 * sin(float(i) * 12.9898)
		var body := StaticBody3D.new()
		body.collision_layer = 1
		var cs := CollisionShape3D.new()
		var bx := BoxShape3D.new()
