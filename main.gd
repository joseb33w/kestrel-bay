...
const CAM_STEP_SMOOTH := 16.0     # e-folds/sec the camera catches up a step-up pop (~0.15 s to settle)
...
var _y_lag := 0.0                       # metres the CAMERA is still behind an absorbed vertical pop
...
func _chunk_physics(delta: float) -> void:
	...
	player.move_and_slide()
	# Everything after move_and_slide() that changes Y does it by ASSIGNMENT, not by velocity — a
	# teleport the renderer has no way to smooth. Remember where the integrator left us so the two
	# writes below can be measured and absorbed (see _y_lag).
	var y_integrated := player.global_position.y
	_step_up_assist(dir)
	...
	_ground_stuck = false
	# ...AND NOT WHEN SOMETHING IS ALREADY HOLDING YOU UP. The comment above says "anything
	# standing ABOVE the ground on a structure/step is left to real physics", but the test was a
	# HEIGHT comparison, and a floor can legitimately sit BELOW the terrain beside it. A building
	# is grounded at its DOOR-side terrain height and its interior slab sits at that base
	# (build_structure.gd, GroundSlab at local y=0) — so wherever the ground rises across the
	# footprint, the player standing on that slab is below _ground_y and the pin fired: teleported
	# up THROUGH the floor onto the hillside outside. Walking uphill indoors did it every time.
	#
	# is_on_floor() is the honest form of the same question, it is fresh here (move_and_slide ran
	# at :755), and it does not weaken the pin's actual job — when the cell's trimesh has not
	# streamed in there is nothing to stand on, so it is false and the pin still catches you.
	if not player.is_on_floor() and player.velocity.y <= 0.5 and player.global_position.y <= floor_y + 0.1:
		player.global_position.y = floor_y
		player.velocity.y = 0.0
		_ground_stuck = true
	_absorb_y_pop(player.global_position.y - y_integrated)
...
## ABSORB A VERTICAL TELEPORT so the camera glides where the body jumps.
##
## The player's Y has three writers and only one of them is physics: move_and_slide() integrates it,
## then _step_up_assist ASSIGNS it onto a step lip, and the ground-stick ASSIGNS it onto the terrain.
## Both assignments are instantaneous by construction — physics interpolation cannot smooth them,
## because from the renderer's point of view nothing moved, the body simply IS somewhere else.
##
## On a staircase that fires once per tread. Ashford's flights are 0.225 m risers, so climbing one
## teleported the camera up 0.225 m several times a second and the whole world appeared to judder
## downwards on every step. That is the "it shakes when it moves up the stairs" report, and it is
## not specific to those stairs: ANY stepped geometry in ANY game does it, which is why the fix
## belongs here rather than in a staircase.
##
## The camera, not the avatar. In third person the body sits under the camera rig, so the pop moves
## the VIEW — the character itself looks stable and the world lurches. Lagging the rig alone removes
## the lurch and leaves the character rising smoothly through frame, which is what climbing should
## look like. Lagging the avatar too would sink its feet into the tread for no gain.
##
## Only upward, and only step-sized. A downward snap is a landing and should read instantly, and
## anything bigger than a step is a real teleport — a spawn, a pit escape, an area swap — which the
## player must SEE happen, so that clears the lag instead of smoothing it into a slide.
func _absorb_y_pop(pop: float) -> void:
	if pop > 0.01 and pop <= STEP_MAX + 0.05:
		_y_lag = minf(_y_lag + pop, STEP_MAX)
	elif absf(pop) > STEP_MAX + 0.05:
		_y_lag = 0.0
...
func _process(delta: float) -> void:
	...
	if cam_rig and player:
		...
		# ...minus whatever vertical POP the physics step just teleported through. Without this the
		# camera is welded to a body that jumps a whole riser in one frame, so climbing stairs reads
		# as the world juddering downwards once per tread. See _absorb_y_pop.
		if _y_lag > 0.0001:
			_y_lag *= exp(-delta * CAM_STEP_SMOOTH)
			if _y_lag < 0.0015:
				_y_lag = 0.0
		cam_rig.global_position = player.get_global_transform_interpolated().origin + Vector3(0.0, head - _y_lag, 0.0)
...
func _build_player() -> void:
	...
	#     GSurf.fix_rigger_materials(avatar)   # Meshy rigged chars are metallic-1.0 without it
